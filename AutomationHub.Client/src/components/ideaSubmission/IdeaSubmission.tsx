...
const IdeaSubmission: React.FC<IdeaSubmissionProps> = ({ className }) => {
  ...
  const { setIsFeedbackDisplayed } = useContext(AppContext);
  const { t } = useTranslation('IdeaSubmission');
  const { width } = useScreenSize();
  const styles = useStyles();

  /**
   * Toggle the inapp feedback dialog display.
   */
  ...
  return (
    <div className={mergeClasses(styles.root, className)}>
      <Card className={styles.card} size="small" onClick={showFeedbackDialog}>
        <CardHeader
          header={
